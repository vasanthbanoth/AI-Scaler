...
# 2. Deploy API & Web (Unified Render Deployment)
...
| **Public chat URL** | `https://<your-render-app>.onrender.com/chat` |
| GitHub | `https://github.com/vasanthbanoth/scaler-ai-persona` |
| Eval PDF | `evals/output/eval_report.pdf` |
| Loom | ≤4 min — architecture + one hard problem (Unified deployment / RAG fallback) |
...
2. **Render** (Unified Backend & Frontend) — Blueprint from `render.yaml` → copy `https://xxx.onrender.com`
3. **Vapi** — `./scripts/configure_vapi.sh` → attach phone
4. **Cal.com** — keys on Render
5. Keep live **7 days** after submit
...
| Live chat | Render unified `/chat` → FastAPI `/chat` |
